...
/**
 * Convert ISO country code or already full name to full country name
 */
if (!function_exists('dtr_convert_country_code_to_name')) {
    function dtr_convert_country_code_to_name($value) {
        $value = trim($value);
        ...
        // ✅ Case 1: Value is a valid ISO code
        if (isset($countries[$upper])) {
            return $countries[$upper];
        }

        // ✅ Case 2: Value already matches a full country name
        if (in_array($value, $countries, true)) {
            return $value;
        }

        // ✅ Case 3: No match — return original for debugging
        return $value;
    }
}
