...
/**
 * Helper: Return preferred -> legacy employers.json paths (absolute)
 */
if (!function_exists('workbooks_employers_json_paths')) {
    function workbooks_employers_json_paths() {
        return [
            // New canonical location under assets/json
            trailingslashit(WORKBOOKS_NF_PATH) . 'assets/json/employers.json',
            // Legacy fallback (plugin root) retained for backward compatibility
            trailingslashit(WORKBOOKS_NF_PATH) . 'employers.json'
        ];
    }
}

/**
 * Helper: Write employers JSON to new path (fallback to legacy). Returns path written or false.
 */
if (!function_exists('workbooks_write_employers_json')) {
    function workbooks_write_employers_json($employers) {
        $json = json_encode($employers, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
        foreach (workbooks_employers_json_paths() as $path) {
            $dir = dirname($path);
            if (!is_dir($dir)) {
                // Suppress errors but attempt directory creation
...
/**
 * Helper: Read employers JSON (tries new then legacy). Returns array or empty array.
 */
if (!function_exists('workbooks_read_employers_json')) {
    function workbooks_read_employers_json() {
        foreach (workbooks_employers_json_paths() as $path) {
            if (file_exists($path)) {
...
function workbooks_save_employers($employers) {
    ...
    // Generate JSON file for frontend dropdown (new path with legacy fallback)
    workbooks_write_employers_json($employers);
...
function workbooks_generate_employers_json_from_db() {
    ...
    // Generate JSON file (new path first, fallback to legacy)
    if (!workbooks_write_employers_json($employers)) {
        return [
            'success' => false,
            'message' => 'Failed to write employers.json file (new or legacy path). Check directory permissions.'
        ];
    }
...
function workbooks_sync_employers_cron() {
    ...
    try {
        do {
            ...
            $all_employers = array_merge(is_array($all_employers) ? $all_employers : [], is_array($employers) ? $employers : []);
            $start += $batch_size;
...
add_action('wp_ajax_fetch_workbooks_organisations', function() {
    ...
    // Check JSON file first (new then legacy)
    $employers = workbooks_read_employers_json();
    if (!empty($employers)) {
    ...
    if (!empty($employers)) {
    // Save to JSON file for future use (new path with legacy fallback)
    workbooks_write_employers_json($employers);
        wp_send_json_success($employers);
        return;
    }
    ...
    try {
        ...
        do {
            ...
            $organisations = $response['data'] ?? [];
            $all_organisations = array_merge(is_array($all_organisations) ? $all_organisations : [], is_array($organisations) ? $organisations : []);
            $start += $batch_size;
        } while (!empty($organisations) && count($organisations) == $batch_size);
        
        if (!empty($all_organisations)) {
            // Save to database and JSON file (handled inside workbooks_save_employers)
...
// Lightweight ping endpoint to confirm file loaded & AJAX routing for unauthenticated visitors
if (!function_exists('workbooks_employers_ping')) {
    function workbooks_employers_ping() {
        workbooks_debug_log('employers_ping hit ('.(is_user_logged_in()?'logged-in':'anon').')');
        wp_send_json_success([
            'message' => 'ok',
    ...
    add_action('wp_ajax_workbooks_employers_ping', 'workbooks_employers_ping');
    add_action('wp_ajax_nopriv_workbooks_employers_ping', 'workbooks_employers_ping');
}
...
// Allow unauthenticated access (public registration form) with nonce
add_action('wp_ajax_nopriv_fetch_workbooks_employers_paged', function() {
    workbooks_debug_log('fetch_workbooks_employers_paged (nopriv) called');

    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'workbooks_nonce')) {
        workbooks_debug_log('Nonce verification failed (nopriv)');
        wp_send_json_error('Security check failed');
        return;
    }
    ...
    global $wpdb; $table_name = $wpdb->prefix . 'workbooks_employers';
    if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") !== $table_name) {
        wp_send_json_error('Employers table does not exist.');
    }
    $query = "SELECT id, name, last_updated FROM $table_name";
    $count_query = "SELECT COUNT(*) FROM $table_name";
    if ($search !== '') {
...
/**
 * Ajax: Select2-friendly employer search (supports anonymous users)
 * Returns: { results: [ { id, text } ], pagination: { more: bool } }
 * Ported from release plugin version for performant, paginated search.
 */
if (!function_exists('workbooks_fetch_employers_select2')) {
function workbooks_fetch_employers_select2() {
    // Allow public (registration form) but still require nonce for basic protection.
    // Accept either legacy 'workbooks_nonce' or earlier mis-matched 'dtr_workbooks_nonce' to be forgiving.
    $nonce = isset($_REQUEST['nonce']) ? $_REQUEST['nonce'] : '';
    $valid = $nonce && ( wp_verify_nonce($nonce,'workbooks_nonce') || wp_verify_nonce($nonce,'dtr_workbooks_nonce') );
    workbooks_debug_log('fetch_employers_select2 entry', [
        'valid_nonce' => $valid ? 1 : 0,
        'have_nonce' => $nonce ? 1 : 0,
        'anon' => is_user_logged_in() ? 0 : 1,
        'params' => [
    ...
    if (!$valid) {
        // Still return 200 with empty dataset (avoid 400) but log it for diagnosis
        workbooks_debug_log('fetch_employers_select2 invalid nonce', ['nonce' => substr($nonce,0,12)]);
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") !== $table_name) {
        workbooks_debug_log('fetch_employers_select2 table missing', $table_name);
        wp_send_json(['results' => [], 'pagination' => ['more' => false]]);
    }

    $search = isset($_REQUEST['term']) ? sanitize_text_field(wp_unslash($_REQUEST['term'])) : '';
    $page   = isset($_REQUEST['page']) ? max(1, intval($_REQUEST['page'])) : 1;
    $limit  = isset($_REQUEST['limit']) ? min(100, max(5, intval($_REQUEST['limit']))) : 30; // sensible defaults
    $offset = ($page - 1) * $limit;
    ...
    if ($search !== '') {
        ...
        $starts   = $wpdb->esc_like($search) . '%';
        $exact    = $search;
        $where    = 'WHERE name LIKE %s';
        $params[] = $wildcard;
    }

    // Build ORDER BY to prioritize: exact match > starts-with > contains alphabetical
    $order_fragments = [];
    if ($search !== '') {
        $order_fragments[] = $wpdb->prepare('(CASE WHEN name = %s THEN 0 ELSE 1 END)', $exact);
        $order_fragments[] = $wpdb->prepare('(CASE WHEN name LIKE %s THEN 0 ELSE 1 END)', $starts);
    }
    $order_fragments[] = 'name ASC';
    $order_by = 'ORDER BY ' . implode(', ', $order_fragments);

    // Main query
    $sql = "SELECT id, name FROM $table_name $where $order_by LIMIT %d OFFSET %d";
    $params[] = $limit;
    $params[] = $offset;
    $prepared = $wpdb->prepare($sql, $params);
    $rows = $wpdb->get_results($prepared, ARRAY_A);
    if ($rows === null) {
        workbooks_debug_log('fetch_employers_select2 query error', $wpdb->last_error);
    }

    // Total for pagination (only if searching or first page) / approximate for large unfiltered lists
    if ($search !== '') {
        $count_sql = "SELECT COUNT(*) FROM $table_name $where";
        $count_prepared = $wpdb->prepare($count_sql, array_slice($params, 0, 1));
        $total = (int)$wpdb->get_var($count_prepared);
    } else {
        $next_check_sql = $wpdb->prepare("SELECT 1 FROM $table_name ORDER BY name ASC LIMIT 1 OFFSET %d", $offset + $limit);
        $has_next = (bool)$wpdb->get_var($next_check_sql);
        $total = $has_next ? $offset + $limit + 1 : $offset + count($rows);
    }
    ...
    workbooks_debug_log('fetch_employers_select2 success', [
...
add_action('wp_ajax_fetch_workbooks_employers_select2', 'workbooks_fetch_employers_select2');
add_action('wp_ajax_nopriv_fetch_workbooks_employers_select2', 'workbooks_fetch_employers_select2');
}
