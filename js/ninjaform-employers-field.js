...
(function($){
    ...
    // New: Dynamically add .workbooks-employers to a select and initialize
    window.makeWorkbooksEmployersSelect = function(selectorOrElement){
        var $el = $(selectorOrElement);
        if(!$el.length) return;
        $el.addClass('workbooks-employers');
        initEmployerSelect($el);
    };

    // Legacy: auto-detect and initialize for Ninja Forms
